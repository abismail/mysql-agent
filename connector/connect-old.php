...
abstract class Connect{
	...
	protected function getResults(){
		...
		while($row = mysql_fetch_array($this->result)){
			...
			$c = 0;
			print "the length of the second dim is: " . count($row) . "<br/>" . $row[0] . "<br/>" . $row[1] . "<br/>";
		...
		mysql_close($con);
		print "printing first element of multidim array: " . $returnArray[2][0] . "<br/>" . $returnArray[2][1] . "<br/>";
		//print_r($returnArray);
		foreach($y = 0; $y < count($returnArray); $y++){
			$array = $returnArray[$y];
			foreach($c = 0; $c < count($array); $c++){
				if(($c / 1) == 0){
				$returnArray[$y][$c] = ;}
			}
		}
